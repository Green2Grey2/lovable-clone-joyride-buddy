...
export const FloatingActivityFeed = () => {
    ...
    return (
      <Button
        ...
        className={`fixed right-4 z-40 h-14 w-14 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg hover:shadow-xl transition-all duration-300 p-0 ${
          isBottomNavVisible ? 'bottom-24 md:bottom-6' : 'bottom-6'
        }`}
      >
  ...
  return (
    <Card className={`fixed right-4 z-40 w-[calc(100vw-2rem)] max-w-md transition-all duration-300 shadow-xl ${
      isMinimized 
        ? `h-14 ${isBottomNavVisible ? 'bottom-24 md:bottom-6' : 'bottom-6'}` 
        : `top-20 md:top-auto h-[calc(100vh-8rem)] md:h-[600px] ${isBottomNavVisible ? 'bottom-24 md:bottom-6' : 'bottom-6'}`
    }`}>

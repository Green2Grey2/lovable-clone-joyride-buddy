-- Fix GROUP BY issue in generate_workout_insights function
...
BEGIN
  ...
  WITH weekly_performance AS (
    SELECT 
      ...
      LAG(AVG(a.calories_burned)) OVER (ORDER BY DATE_TRUNC('week', a.date)) as prev_avg_calories
    FROM public.activities a
  ...
  SELECT 
    CASE 
      WHEN COUNT(*) >= 2 THEN
        CASE 
          WHEN EXISTS (SELECT 1 FROM weekly_performance WHERE prev_avg_calories IS NOT NULL AND avg_calories > prev_avg_calories) THEN 'improving'
          WHEN EXISTS (SELECT 1 FROM weekly_performance WHERE prev_avg_calories IS NOT NULL AND avg_calories < prev_avg_calories) THEN 'declining'
          ELSE 'stable'
        END

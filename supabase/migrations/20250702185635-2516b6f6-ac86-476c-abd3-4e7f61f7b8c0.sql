...
-- Insert some default challenges if they don't exist
INSERT INTO public.challenges (title, description, type, start_date, end_date, target_value, created_by, is_active) 
SELECT 'Weekly Step Challenge', 'Complete 50,000 steps this week', 'steps', CURRENT_DATE, CURRENT_DATE + INTERVAL '7 days', 50000, user_id, true
FROM profiles LIMIT 1
WHERE NOT EXISTS (SELECT 1 FROM challenges WHERE title = 'Weekly Step Challenge');

INSERT INTO public.challenges (title, description, type, start_date, end_date, target_value, created_by, is_active)
SELECT 'Monthly Distance Challenge', 'Walk 100 miles this month', 'distance', DATE_TRUNC('month', CURRENT_DATE), DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month' - INTERVAL '1 day', 100, user_id, true
FROM profiles LIMIT 1
WHERE NOT EXISTS (SELECT 1 FROM challenges WHERE title = 'Monthly Distance Challenge');